...
const Invoice = ({ caseData }: { caseData: any }) => {
  ...
  return (
    <div className="relative max-w-4xl overflow-hidden mx-auto bg-white p-8 shadow-lg">
      ...
      <div className="mx-auto p-8" ref={targetRef}>
        <h1 className="w-full py-6 text-center text-3xl  tracking-wider text-blue-600">
          <span className="text-gray-700">JET</span>
          <span className="text-orange-400"> PASSPORTS</span>
        </h1>

        <table className="mb-8 w-full">
          <tbody>
            <tr className="flex flex-col md:flex-row">
              <td className="w-full md:w-2/3 text-xs md:text-sm tracking-wider text-gray-600">
                Private U.S. Based Company Registered With The Department Of
                State To Expedite US Passports.
              </td>
              <td className="w-full md:w-1/3 text-left mt-3 md:mt-0 md:text-right tracking-wider">
                <p className="break-all font-semibold tracking-widest">
                  Receipt for Order # {caseData.caseNo}
                </p>
                <p className="text-xs md:text-sm">9:00 AM to 6:00 PM EST</p>
                <p className="text-xs md:text-sm">{contactInfo.phone}</p>
                <p className="text-xs md:text-sm">{contactInfo.email}</p>
              </td>
            </tr>
          </tbody>
        </table>
        <div className="flex flex-col md:flex-row w-full justify-between">
          <div>
            <h2 className="mb-4 text-base md:text-xl font-semibold tracking-wider">
              Order Information
            </h2>
                ...
                <React.Fragment key={service._id}>
                  <tr>
                    {/* <td className="p-2 tracking-wider">
                      {new Date(caseData.).toLocaleDateString()}
                    </td> */}
        ...
        <div className="text-xs md:text-sm tracking-wider text-gray-600">
          <p>
          ...
          <p className="mt-2">
            www.jetpassport.com | Chicago Passport & Visa Expedite Service {contactInfo.phone}
          </p>
          <p>

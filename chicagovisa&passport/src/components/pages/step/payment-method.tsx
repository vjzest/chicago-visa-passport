...
export function CardsPaymentMethod({
    ...
    try {
      ...
      const { data } = await axiosInstance.post(
        "/common/cases",
        {
          ...
          billingInfo: { ...billingData, expirationDate: formattedExpiry },
          accountDetails,
          // applicationDetails: {
  ...
  return (
    <>
      ...
      <Form {...form}>
        <form className="mt-0  " onSubmit={submitPaymentForm}>
          ...
          <CardFooter className="!mx-0 !px-0 ">
            {isPaymentProcessing ? (
              ...
              <Button disabled={!tncAccepted} className="w-full" type="submit">
                Process my order for ${generalFormData?.totalAmount?.toFixed(2)}
              </Button>
            )}
          </CardFooter>
        </form>

...
const Requirement = () => {
  ...
  const renderPoint = (point: string, currentItem: any) => {
    // First split for DS-3053
    const parts = point.split(/(DS-3053)/);

    // Further split each non-DS-3053 part to handle the word "form" in item 7
    const processedParts = parts.map((part) => {
      ...
      // For item 7, split and process the word "form"
      if (point.includes("form") && currentItem.id === 7) {
        const formParts = part.split(/(form)/i); // Case insensitive match
        return formParts.map((formPart, index) => {
          if (formPart.toLowerCase() === "form") {
            return (
              <a
                key={`form-${index}`}
                href="/Social.pdf"
                ...
                {item.description && (
                  <div className="p-3">
                    <p className="text-[#666666] text-sm">
                      {renderPoint(item.description, item)}
                    </p>
                  </div>
                )}

...
// Configuration for states that can be split into regions
...
export const getCountryName = (code: string) => {
  ...
  if (country) {
    countryNameCache[code] = country.name;
    return country.name;
  }
  return "";
};
...
export const getCountryCode = (name: string) => {
  if (countryCodeCache[name]) return countryCodeCache[name];

  const country = countries.find((country) => country?.name?.toLowerCase() === name?.toLowerCase());
  if (country) {
    countryCodeCache[name] = country.code;
    return country.code;
  }
  return "";
};

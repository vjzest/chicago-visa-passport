...
import ConsularFeeService from "./consular-fee.service";
...
export default class CasesService {
    ...
    try {
      ...
      console.log({
        ...
        processorCreds: {
          ...
          processorName: processorDetails?.processorName as string,
        },
      });
  ...
  async sendTestimonialRequest(
    ...
    try {
      ...
      const userEmailContent = `
          ...
          <p>Thank you for choosing Chicago Passport & Visa Expedite Services for your passport expediting needs. We hope you found our service simple, fast, and convenient. Your feedback is invaluable, and we’d love to hear about your experience.</p>
          ...
          <p>Chicago Passport & Visa Expedite Services</p>
        `;
      ...
      const userEmailData = {
        ...
        subject: "Thank You For Choosing Chicago Passport & Visa Expedite Services",
  ...
  async chargeForOrder(
    ...
    try {
      ...
      const citizenOf = (caseDetails as any)?.citizenOf;
      const travelingTo = (caseDetails as any)?.travelingTo;
  ...
  async toggleAutoEmails(caseId: string): ServiceResponse {
    try {
      ...
      await caseDetails.save();
      return {
        message: `Auto emails ${caseDetails.caseInfo.disableAutoEmails ? "enabled" : "disabled"
          } successfully`,
        success: true,
        status: 200,
      };

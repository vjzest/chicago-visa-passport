...
export default class AdminSSOService {
    ...
    async verifyCode(code: string): Promise<ServiceResponse> {
        try {
            ...
            if (ssoUser.projectSlug !== "chicago-visa" && ssoUser.projectSlug !== "chicago-visa-passport") {
            ...
            if (!admin) {
                ...
                // For now, we assume roles are synced or we pick a default
                // Find role by name or ID from SSO
                // Try "Admin" (Capitalized) then "admin" (lowercase)
                let role = await this.rolesModel.findOne({ title: "Admin" });
                if (!role) {
                ...
                admin = await this.adminModel.create({
                    ...
                    password: randomUUID(), // SSO handles auth, local pwd unused but required by schema
                    role: role._id,
                    status: "Active",
                    autoAssign: false,
                });
            ...
            // 3. Generate Local JWT (Parity with AdminAuthService)

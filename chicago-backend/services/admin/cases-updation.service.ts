    ...
    try {
      ...
      if (
        ...
        if (prevServiceLevelAmount >= newServiceLevelAmount) {
          ...
          const paymentEmailContent = `
            ...
            2
          )} for upgrading your service level to '${newServiceLevel?.serviceLevel
            }</p>
            <p>If you have any questions, please contact our support team.</p>
          `;
          //@ts-ignore
        ...
        const emailContent = `
          ...
          <p>Dear ${getFullNameFromAccountDetails(case_?.account)},</p>
          <p>We would like to inform you that your service level has been upgraded from '${prevServiceLevel?.serviceLevel
        ...
        if (currentStatus?.key === "passport-processed") {
          ...
          const shippingData: ShipmentDetails = {
            ...
            recipients: [
              {
                contact: {
                  personName: `${(case_ as any)?.account?.firstName} ${(case_ as any)?.account?.lastName
                    }`,
                  companyName: "NONE",
      ...
      if (
        ...
        const prevServiceType = prevAndCurrentServiceTypes.find(
          (serviceType) =>
            (serviceType?._id as unknown as ObjectId).toString() ===
            case_.caseInfo.serviceType?.toString()
        );
        const currentServiceType = prevAndCurrentServiceTypes.find(
          (serviceType) =>
            (serviceType?._id as unknown as ObjectId).toString() === data.serviceType
        );
        systemNotes.push(
        ...
        const emailContent = `
          ...
          <p>We would like to inform you that your service type has been updated from '${prevServiceType?.serviceType
  ...
  async patchCase(
    ...
    try {
      ...
      updatedCase?.notes.push({
        manualNote: "",
        autoNote: `Case field updated ${camelCaseToNormalCase(fieldKey)} to ${value instanceof Date ? value.toLocaleDateString() : value
          } by <strong>${adminName}</strong>`,
        host: "system",
        createdAt: new Date(),
      });
  ...
  async cancelCase(
    ...
    try {
      ...
      const caseManagerEmailContent = `
        <h2>Passport Application ${action.charAt(0).toUpperCase() + action.slice(1)
        }</h2>
        <p>Dear ${caseManagerName},</p>
        <p>We would like to inform you that the Passport application with caseNo ${currentCase?.caseNo
        } has been ${action}.</p>
        ${isCanceled
      ...
      const caseManagerEmailData = {
        ...
        subject: `Passport Application ${action.charAt(0).toUpperCase() + action.slice(1)
          }`,
      ...
      const userEmailContent = `
        ...
        <p>Dear ${currentCase?.account?.firstName + " " + currentCase?.account?.lastName
        },</p>
        <p>We ${isCanceled ? "regret" : "greed"
      ...
      const userEmailData = {
        ...
        fullName: `${currentCase?.account?.firstName + " " + currentCase?.account?.lastName
          }`,
        subject: `Passport Application ${action.charAt(0).toUpperCase() + action.slice(1)
          }`,
        htmlContent: userEmailContent,
  ...
  async confirmInboundShipment(
    ...
    try {
      ...
      const shippingData: ShipmentDetails = {
        shipper: {
          contact: {
            personName: `${(caseDoc?.account as any)?.firstName} ${(caseDoc?.account as any)?.lastName
              }`,
            companyName: "NONE",

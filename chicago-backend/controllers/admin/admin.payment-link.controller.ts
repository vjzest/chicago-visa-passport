...
class AdminPaymentLinkController {
    ...
    async generate(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            // Check if it's a passport service to route to the correct application page
            if (serviceType?.silentKey?.toLowerCase()?.includes('passport')) {
                path = '/us-passport/apply';
            }

            const paymentUrl = `${baseUrl}${path}?type=directLink&token=${token}`;

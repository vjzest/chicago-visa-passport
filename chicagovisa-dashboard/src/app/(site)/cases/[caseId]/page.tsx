...
import { CaseForm } from "@/components/pages/cases/case-form";
...
import CaseDetails from "@/components/pages/cases/case-id/case-details";
import LoadingPage from "@/components/globals/LoadingPage";
import NotesList from "@/components/pages/cases/note-list";
import BreadCrumbComponent from "@/components/globals/breadcrumb";
import ExportNotesButton from "@/components/pages/cases/note-download";
...
import ChatModal from "@/components/pages/cases/chat-modal";

import Link from "next/link";
import DocumentVerificationSection from "@/components/pages/cases/document-verification-section";
import CourierNoteForm from "@/components/pages/cases/courier-note";
import { toast } from "sonner";
import ActionNotes from "@/components/pages/cases/action-note";
import { useAdminStore } from "@/store/use-admin-store";
import CustomAlert from "@/components/globals/custom-alert";
import AccountDetailsCard from "@/components/pages/cases/account-details-card";
import { useRouter } from "next/navigation";
import OrderChargeForm from "@/components/pages/cases/order-charge-form";
import { CaseFiles } from "@/components/pages/cases/case-files";
  ...
  function renderPageFromFields() {
    ...
    function recursiveRender(obj: any, title?: string) {
      ...
      for (const key in obj) {
        ...
        // Handle Billing Info specially for Offline Links or missing card info
        if (key === "billingInfo") {
          const isOffline = caseDetails?.isOfflineLink || (!obj[key]?.cardNumber && !obj[key]?.cardHolderName);
          if (isOffline) {
            if (title || key === "billingInfo") { // Ensure title renders if top level or needed
              jsxArray.push(
                <h2 key="billing-title" className="mt-4 w-fit border-b-2 border-dashed text-xl font-semibold text-slate-600">
                  Billing Info
                </h2>
              );
            }
            ...
            continue; // Skip recursive render of empty card fields
          }
        }
                ...
                (caseDetails.applicationReviewStatus === "ongoing" ? (
                  ...
                  <>
                    <p className="text-center font-medium text-slate-500">
                      User has not submitted their Visa details for review yet.
                    </p>

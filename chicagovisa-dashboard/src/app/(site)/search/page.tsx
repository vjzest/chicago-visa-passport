...
type IFilter = {
  ...
  reworkNeeded?: string;
  keywords?: string;
  orderId?: string; // Map caseId to orderId for CRM
};
...
export default function SearchPage() {
  ...
  const fetchStatusesAndLocations = async () => {
    try {
      const [statusesResponse, locationsResponse, serviceTypesRes] = await Promise.all([
  ...
  const searchFunction = async (filters: IFilter) => {
    ...
    try {
      ...
      if (crmMode) {
        ...
        if (filters.visaCountry) queryParams.append("visaCountry", filters.visaCountry);
        if (filters.expediteTier) queryParams.append("expediteTier", filters.expediteTier);
        if (filters.docSubmissionDate) queryParams.append("docSubmissionDate", filters.docSubmissionDate);
        if (filters.acquisitionSource) queryParams.append("acquisitionSource", filters.acquisitionSource);
        if (filters.campaign) queryParams.append("campaign", filters.campaign);
        if (filters.landingPage) queryParams.append("landingPage", filters.landingPage);
        if (filters.courierLocation) queryParams.append("courierLocation", filters.courierLocation);
        if (filters.qaStatus) queryParams.append("qaStatus", filters.qaStatus);
        if (filters.reworkNeeded) queryParams.append("reworkNeeded", filters.reworkNeeded);
        if (filters.keywords) queryParams.append("keywords", filters.keywords);
        // Map caseId to orderId
        if (filters.caseId) queryParams.append("orderId", filters.caseId);
      }
  ...
  return (
    <>
      ...
      <div className="flex justify-end items-center gap-2 mb-4 px-4 md:px-32">
        <Label htmlFor="crm-mode" className="font-semibold text-deep-blue">Advanced CRM Search</Label>
        <Switch id="crm-mode" checked={crmMode} onCheckedChange={setCrmMode} />
      </div>

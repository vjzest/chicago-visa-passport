  ...
  const onSubmit = async () => {
    try {
      ...
      if (formData.title.trim().length < 3) {
        setFormError((prev) => ({
          title: "Please enter a valid role name with atleast 3 characters!",
          access: prev.access,
        }));
        window.scrollTo(0, 0);
        return;
      ...
      if (
        ...
        setFormError((prev) => ({
          title: prev.title,
          access: "Please select atleast one access permission!",
        }));
        window.scrollTo(0, 0);
        return;
      }
      ...
      const { data } = await axiosInstance.post("/admin/roles", formData);
      if (!data?.success) throw new Error(data?.message);
      toast.success(data?.message);
      router.push('/passport/manage-roles');
    } catch (error) {
      console.log(error);
    ...
    <>
      <BreadCrumbComponent
        customBreadcrumbs={[
          ...
          { label: "Add role", link: null },
        ]}
      />
      <h1 className="text-xl font-semibold md:text-2xl mb-4">Add role</h1>
      <Card className="p-4">
        <form
          onSubmit={(e) => {
            e.preventDefault();

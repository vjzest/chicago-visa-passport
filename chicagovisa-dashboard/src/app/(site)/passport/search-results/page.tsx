  ...
  const fetchCases = async () => {
    ...
    try {
      ...
      if (
        ...
        queryObj.statuses.length === 0 ||
        queryObj.processingLocations.length === 0
      ) {
      ...
      const temp = data.data?.map((item: ICaseListing) => {
        return {
          ...
          firstName: item.applicantInfo?.firstName,
          middleName: item.applicantInfo?.middleName,
          lastName: item.applicantInfo?.lastName,

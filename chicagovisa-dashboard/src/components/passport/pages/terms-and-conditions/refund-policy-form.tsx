...
const RefundPolicyForm: React.FC<{}> = () => {
  ...
  const onSubmit = async () => {
    ...
    try {
      setIsDisabled(true);
      await axiosInstance.put("/admin/configs/refund-policy", {
        content,
      });
  ...
  const fetchTnc = async () => {
    try {
      const response = await axiosInstance.get("/admin/configs/refund-policy");
      ...
      <CardContent className="flex flex-col p-4">
        <Dialog open={showConfirmation} onOpenChange={setShowConfirmation}>
          <DialogContent>
            ...
            <DialogDescription>
              Are you sure you want to update the refund policy?
            </DialogDescription>
        ...
        <form className="flex h-auto flex-col gap-4 py-4">
          <div className="flex justify-between whitespace-nowrap">
            <label htmlFor="user-tnc">Refund Policy</label>
            {expand ? (

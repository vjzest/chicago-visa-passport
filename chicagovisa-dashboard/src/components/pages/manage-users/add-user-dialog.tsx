...
const AddUserDialog: React.FC<AddUserDialogProps> = ({
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      if (values.passportAccess) {
        ...
        if (passportRole) {
          projectAccess.push({
            project: "jet-passport",
            roleId: passportRole._id,
            enabled: true,
          });
        }
      }
      ...
      // Attempt Dual Save (using clean axios to avoid interceptor token mismatch)
      ...
      // Chicago save
      if (chicagoToken) {
        const chicagoPromise = axios.post(`${chicagoUrl}/admin/admins`, getFormData(), {
          headers: {
            Authorization: `Bearer ${chicagoToken}`,
            "Content-Type": "multipart/form-data"
          }
        });
        promises.push(chicagoPromise);
      }

      // Passport save
      if (passportToken) {
        const passportPromise = axios.post(`${passportUrl}/admin/admins`, getFormData(), {
          headers: {
            Authorization: `Bearer ${passportToken}`,
            "Content-Type": "multipart/form-data"
          }
        });
        promises.push(passportPromise);
      }
      ...
      if (errors.length === 1) {
        const failedSystem = results[0].status === "rejected" ? "Chicago" : "Passport";
        toast.warning(`Saved partially. Failed to sync with ${failedSystem}.`);
      } else {
  ...
  const fetchRoles = async () => {
    try {
      ...
      const [chicagoRes, passportRes] = await Promise.all([
      ...
      if (chicagoRes.data?.success) setRoles(chicagoRes.data.data);
      if (passportRes.data?.success) setPassportRoles(passportRes.data.data);
    } catch (error) {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="py-3 col-span-2">
              ...
              >
                Click to choose{" "}
                {image || field.value ? "another image" : "profile image"}
              </FormLabel>
              <FormControl>
                <Input
                  {...field}
                  value={""} // Always set the value to an empty string for file inputs
                  hidden
                  onChange={(e) => {
                    ...
                    if (file) {
                      ...
                      setImage(null);
                      field.onChange(null); // Clear the value in React Hook Form
                    }
                  }}
                  type="file"
                  accept="image/*"
              ...
              <FormControl>
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    field.onChange(formattedName);
                    form.setValue("email", formattedName);
                  }}
                  value={field.value ?? ""}
                  className="mt-1 block w-full rounded-md border border-gray-300 p-2"
                />
              ...
              <FormItem className="">
                <FormControl>
                  <div className="flex items-center gap-3">
                    <FormLabel htmlFor="ipRestriction">
                      IP Restriction
                    </FormLabel>
                    <Switch
                ...
                <FormMessage>
                  {form.formState.errors.ipRestriction?.message}
                </FormMessage>
              </FormItem>
            )}
          />

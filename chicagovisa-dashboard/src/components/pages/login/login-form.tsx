  ...
  const onSubmit = async (data: LoginFormInputs) => {
    try {
      ...
      try {
        ...
        passportRes = await axiosInstance.post(`${passportUrl}/admin/auth/login`, loginPayload);
        if (passportRes.status === 200) {
          localStorage.setItem("admin_token_passport", passportRes?.data?.data?.token);
      ...
      if ((chicagoRes && chicagoRes.status === 200) || (passportRes && passportRes.status === 200)) {
        await initialize();

...
import { Consulate } from "@/data/visa-jurisdictions";
import { normalizeStateEntryForDisplay } from "@/lib/jurisdiction-utils";
...
const formSchema = z.object({
  ...
  jurisdictionId: z
    .string()
    .min(1, { message: "Jurisdiction must be selected" }),
  locationName: z
    .string()
...
interface JurisdictionAddressFormProps {
...
export function JurisdictionAddressForm({
  initialValues,
  onSubmit,
  countryPairId,
  consulates,
}: JurisdictionAddressFormProps) {
  ...
  useEffect(() => {
    if (selectedJurisdictionId) {
      const selectedConsulate = consulates.find(
        (c) => c.id === selectedJurisdictionId
      );
      if (selectedConsulate) {
        // Filter US_STATES to only include states in this jurisdiction
        const jurisdictionStates = US_STATES.filter((state) =>
          selectedConsulate.states.some((stateEntry) => stateEntry.state === state.name)
        );
        setAvailableStates(jurisdictionStates);

        // If the currently selected state is not in the jurisdiction, clear it
        const currentState = form.getValues("state");
        if (
          currentState &&
          !jurisdictionStates.find((s) => s.code === currentState)
        ) {
          form.setValue("state", "");
        }
      }
  ...
  }, [selectedJurisdictionId, consulates, form]);
    ...
    <Form {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Jurisdiction (Consulate) *</FormLabel>
              <FormControl>
                <Select

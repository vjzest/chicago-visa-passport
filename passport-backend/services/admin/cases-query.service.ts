...
export default class CasesQueryService {
  ...
  async findOneByCaseIdAndNo(
    ...
    try {
      console.log(`[DEBUG] Service findOneByCaseIdAndNo - Querying DB for ID: ${caseId}, No: ${caseNo}`);
      let caseDetail: any = await this.casesModel
      ...
      console.log(`[DEBUG] Query Result:`, caseDetail ? "Found" : "Not Found");

      if (!caseDetail) {
        console.log(`[DEBUG] Strict lookup failed. Trying fallback by caseNo: ${caseNo}`);
        caseDetail = await this.casesModel
          .findOne({ caseNo })
          .populate([
            { path: "account", model: "accounts" },
            { path: "caseInfo.caseManager", model: "admins" },
        ...
        if (caseDetail) {
          console.log(`[DEBUG] Fallback lookup SUCCESS. Found case._id: ${caseDetail._id}`);
        }
      }

      if (!caseDetail) {
        console.log(`[DEBUG] Case definitely not found.`);
        return {
          status: 404,
          success: false,
          message: "Case not found",

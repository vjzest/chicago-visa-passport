  ...
  async login(email: string, password: string): Promise<ServiceResponse> {
    try {
      ...
      const user = await this.model
        .findOne({ email1: email })
        .exec();
      if (!user) {
        console.log(`[DEBUG] User not found for email: ${email} (Active Check: true)`);
        return {
          status: 404,
  ...
  async sendVerification(email: string, caseId: string): ServiceResponse {
    try {
      ...
      const emailData = {
        to: email,
        fullName,
        subject: "Verify Your Account - Jet Passports",
        htmlContent: `
          <div style="font-family: Arial, sans-serif; color: #333;">
            <h2 style="color: #0056b3;">Welcome to Jet Passports!</h2>
            <p>Dear ${fullName},</p>
            <p>Thank you for registering with Jet Passports. To complete your account setup, please click the link below to verify your email:</p>
  ...
  async sendForgotOtp(email: string, fullName: string): ServiceResponse {
    try {
      ...
      const smsData = {
        to: email,
        fullName: email,
        subject: "Your Jet Passports OTP for Password Recovery",
        htmlContent: `
          <div style="font-family: Arial, sans-serif; color: #333;">
            <h2 style="color: #0056b3;">Forgot Your Password?</h2>
            <p>Dear ${fullName},</p>
            <p>We received a request to reset the password for your Jet Passports account. To proceed with the password reset, please use the One-Time Password (OTP) provided below:</p>
            <div style="text-align: center; margin: 20px 0;">
  ...
  async newPassword(newPassword: string, email: string): ServiceResponse {
    try {
      ...
      const emailData = {
        ...
        subject: "Your Jet Passports Account Password has been Updated",
        htmlContent: `
          <div style="font-family: Arial, sans-serif; color: #333;">
            <h2 style="color: #0056b3;">Password Updated Successfully</h2>
            <p>Dear ${account.firstName + " " + account.lastName},</p>
            <p>Your password has been successfully updated for your Jet Passports account. Please use the new credentials below to log in:</p>

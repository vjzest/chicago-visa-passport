...
export default class CasesService {
  ...
  async create(
    ...
    try {
      ...
      let paymentResponse: any = { success: false, message: "Payment failed" };

      // CHECK FOR OFFLINE PAYMENT TOKEN
      const offlineToken = caseData?.billingInfo?.offlinePaymentToken || caseData?.token;

      if (offlineToken) {
        console.log("Processing offline payment with token:", offlineToken);

        // Use top-level PaymentLinkModel import


        const paymentLink = await PaymentLinkModel.findOne({ token: offlineToken });

        if (!paymentLink) {
          throw new Error("Invalid offline payment token");
        }
        if (paymentLink.status !== 'active') {
          throw new Error(`Payment link is ${paymentLink.status}`);
        }
        ...
        // Validate amount match if needed, or just accept.
        // Mark link as used
        paymentLink.status = 'used';
        paymentLink.caseId = newCase._id as any;
        await paymentLink.save({ session });
        ...
        paymentResponse = {
          ...
          usedProcessorId: null, // Avoid Invalid ObjectId error
          notes: [{
            autoNote: `Payment completed via Offline Link. Token: ${paymentLink.token}`,
            host: "system",
            createdAt: new Date()
          }],
          promoCode: "",
      ...
      //increment the payment processor usage record
      ...
      console.log(`[DEBUG] User ID: ${existingUser._id}, Active: ${existingUser.isActive}`);
      if (newCase) console.log(`[DEBUG] Case ID: ${newCase._id}, CaseNo: ${newCase.caseNo}`);
